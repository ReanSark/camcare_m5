// /dashboard/receptionist/appointments/new/page.tsx
'use client';

import { useEffect, useState } from 'react';
import { useRouter } from 'next/navigation';
import { toast } from 'sonner';
import { ID } from 'appwrite'; // ✅ Required for unique document creation
...
export default function NewAppointmentPage() {
  ...
  // ✅ Fetch both patients and doctors on load
  useEffect(() => {
    const fetchPatientsAndDoctors = async () => {
      try {
        ...
        // ✅ Manual remap for Patient
        setPatients(
          p.documents.map((doc): Patient => ({
            $id: doc.$id,
        ...
        // ✅ Manual remap for Doctor
        setDoctors(
          d.documents.map((doc): Doctor => ({
            $id: doc.$id,
            fullName: doc.fullName,
            role: doc.role,
            userId: doc.userId,
  ...
  // ✅ Handle all single field input changes
  const handleChange = (
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await databases.createDocument(
        ...
        ID.unique(), // ✅ Fix: use proper unique ID
        form
      );
